...
//
// Lifecycle:
//   - Hub created once at startup (cmd/api/main.go) → NewHub
//   - HTTP /ws upgrade calls Accept then Register; Connection takes over the
//     socket
...
// NewHub returns a freshly initialized Hub. Caller MUST call Close() at
// graceful shutdown to drain connections cleanly.
...
// Publish fan-outs the event to every active connection. Drops on per-conn
// channel overflow rather than blocking — slow clients get out-of-sync,
// but they re-sync on next REST fetch.
//
