...
| **PR-PROD4: UX Polish & Bug Bash (Faz 11)** | ⚠️ **DEVAM EDİYOR** — Temel bileşenler oluşturuldu: `web/src/components/ui/empty-state.tsx` (EmptyState: icon+title+description+action, sm/default size, role=status, WCAG), `web/src/components/layout/skip-link.tsx` (SkipLink: ana içeriğe atla, WCAG 2.4.1), `web/src/hooks/use-document-title.ts` (her route'a sayfa başlığı, WCAG 2.4.2), `web/src/components/onboarding/onboarding-tour.tsx` (OnboardingTour: 4 adımlı wizard, localStorage persist, focus trap, ESC kapatma, useOnboardingTour hook). **YAPILMASI GEREKENLER:** (1) App.tsx'e SkipLink + OnboardingTour entegrasyonu, (2) app-shell.tsx ana içerik alanına `id="main-content"` eklenmesi, (3) inventory/index.tsx + admin sayfalarına EmptyState entegrasyonu, (4) useDocumentTitle hook'unun tüm sayfalara eklenmesi, (5) admin/users.tsx + audit-log.tsx'e axe-core ARIA düzeltmeleri. worktree: `.claude/worktrees/pr-prod4` | 🔄 Devam ediyor |
