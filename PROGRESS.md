...
Son güncelleme: 2026-06-03 (PR-DP06 tamamlandı — Entity Detail URL routing)

## Mevcut Durum

- **Önceki Fazlar (Faz 0–11):** 27/27 PR tamamlandı ✅
- **Developer Portal Dönüşümü (DP Fazı):** 6/15 PR tamamlandı
- **Son tamamlanan:** PR-DP06 (Entity Detail URL — /catalog/:kind/:namespace/:name, EntityHeader)
...
| **PR-AI: MCP Server + AI Suggestions** | migration 00055 (ai_suggestions: item_id+type+payload JSONB+accepted_at+rejected_at, UNIQUE constraint, partial index); `internal/llm/client.go` LLM client (Anthropic + OpenAI-compatible, model defaults, field-value-safe prompt — NO secret fields); `internal/config` 4 new env vars (LLMProvider/APIKey/BaseURL/Model); `httpapi/ai_suggestions.go` AISuggestionHandlers (POST /suggest → LLM call + persist ON CONFLICT; GET /suggestions; POST /accept|/reject + audit); `cmd/mcp/main.go` JSON-RPC 2.0 stdio server (6 read-only tools: inventory_search, inventory_get_item, inventory_list_folders, relationships_graph, health_score, audit_search); router wiring; web: `api/ai-suggestions.ts` (4 hooks), item-detail.tsx "✨ AI Önerileri" tab (PII notice + Öneri Al button + SuggestionCard accept/reject); 7 test (LLM: 5 + compile guards: 2) | ✅ Tamamlandı |
| **PR-ANSIBLE: Ansible Dynamic Inventory** | migration 00056 (api_tokens: user_id+name+token_hash BYTEA UNIQUE+scope check+expires_at+last_used_at); `httpapi/ansible_inventory.go` AnsibleInventoryHandlers GetInventory GET /api/v1/ansible/inventory?group_by=tag&filter= (JWT or API token auth, non-secret field keys only — values are E2E encrypted, full Ansible JSON format with _meta+groups+all); `httpapi/api_tokens.go` APITokenHandlers (GET/POST/DELETE /api/v1/users/me/api-tokens, SHA-256 hash stored, plaintext returned once, audit api_token.created/revoked); router wiring + main.go; web: `api/api-tokens.ts` (useAPITokensQuery + create/delete mutations); 3 test (compile guards + default groupBy + scope validation + inventory structure) | ✅ Tamamlandı |
| **PR-ALERT: Prometheus AlertRules** | `deploy/k8s/prometheus-rules.yaml` PrometheusRule CRD (5 alert groups: credentials_expiring_soon/expired, items_unhealthy_high, high_auth_failure_rate, break_glass_active, api_high_error_rate, api_high_latency); `server/internal/metrics/metrics.go` 5 new exported vars: CredentialsExpiringTotal (gauge, "7d"/"30d"), CredentialsExpiredTotal (gauge), ItemsUnhealthyTotal (gauge, "high"/"medium"), BreakglassLoginsTotal (counter), IronStockAuthFailuresTotal (counter with "ironstock_" prefix for rule matching); 3 test (nil check + label validation) | ✅ Tamamlandı |
| **PR-SCIM: SCIM 2.0 Provisioning** | migration 00057 (users.scim_external_id TEXT UNIQUE NULLABLE + partial index); `httpapi/scim_handlers.go` SCIMHandlers — ServiceProviderConfig GET /scim/v2/ServiceProviderConfig; Users: List+Get+Create+PATCH+Delete at /scim/v2/Users; Groups: List+Get+Create+PATCH at /scim/v2/Groups; Bearer token auth via api_tokens scope='scim'; soft-deactivate on DELETE (revokes sessions, no hard-delete); Azure AD bulk PATCH support; `httpapi/scim_export_test.go` white-box test exports; router wiring + main.go scimHandlers; web: `pages/admin/scim.tsx` SCIM Provisioning admin page (SCIM URL display + copy, token create with one-time reveal, token list/delete); App.tsx route + app-shell.tsx sidebar nav; 5 test (compile guard + filter helpers + valueEq parser) | ✅ Tamamlandı |
| **PR-PROD1: CI Security Scanning (Faz 11)** | `.github/workflows/security.yml` 8-job paralel güvenlik workflow (gosec SAST + SARIF upload, govulncheck Go CVE, Trivy FS/container HIGH+CRITICAL, npm audit --audit-level=high, Semgrep multi-dil SAST + SARIF, Gitleaks gizli bilgi tespiti, Kubesec K8s manifest puanlama, CLI govulncheck); nightly cron 02:00 UTC; SARIF GitHub Security tab entegrasyonu; `.github/dependabot.yml` 6 ecosystem haftalık güncelleme (server/cli go.mod, web/root npm, github-actions, docker); `.pre-commit-config.yaml` ESLint web hook eklendi (gitleaks zaten mevcuttu) | ✅ Tamamlandı |
| **PR-CLI: ironstock Go CLI binary (Faz 10)** | `cli/` monorepo dizini — bağımsız Go modülü (ironstock.app/cli); cobra komut ağacı: login (TOTP destekli), logout (--purge), get (--field, --clip, 30s auto-clear), search (--fuzzy, --json), list (folders/items), create item, update item (--field key=value), relationship (add/list), export (ZIP stream), version; `internal/config` (config.json + .tokens 0600 dosya tabanlı token saklama, XDG-uyumlu); `internal/client` (HTTP client, 401'de otomatik token refresh, JSON/APIError yardımcıları); `.goreleaser.yml` (linux/amd64+arm64, darwin/amd64+arm64, windows/amd64, CGO_ENABLED=0); `docs/integrations/cli.md`; 7 test (command tree + isUUID + findFieldValue × 4 + globalFlags) | ✅ Tamamlandı |
| **PR-PROD4: UX Polish & Bug Bash (Faz 11)** | ✅ Temel bileşenler + entegrasyon tamamlandı: `SkipLink` App.tsx'e eklendi (WCAG 2.4.1), `OnboardingTour` App.tsx'e entegre (authenticated route, localStorage persist), `id="main-content" + tabIndex={-1}` app-shell.tsx `<main>` elemanına eklendi, `useDocumentTitle` 33 sayfaya eklendi (WCAG 2.4.2), `EmptyState` bileşeni folder-tree + item-list + item-detail'e entegre edildi, pre-existing test hataları düzeltildi (useItemDuplicatesQuery + useUpdateWebAuthnRequirementMutation mock'ları). Build + 131 test PASS. | ✅ Tamamlandı |
| **PR-PROD3: E2E Test Suite (Faz 11)** | `e2e/` dizini — Playwright @1.44 test altyapısı; `playwright.config.ts` (chromium+firefox, workers=1, trace/screenshot/video CI, 30dk globalTimeout); `tests/global-setup.ts` (API health bekleme, bootstrap admin, TOTP gate bypass, storageState kaydetme); `tests/fixtures.ts` (adminPage/apiToken fixture'ları, createItemViaAPI/createUserViaAPI/loginAs/deleteItemViaAPI yardımcıları); 10 test dosyası: 01-bootstrap-admin (4 test: unauth redirect, login, username görünümü, logout), 02-item-lifecycle (4 test: UI create, list, detail, delete), 03-group-permissions (4 test: before-grant, grant, after-grant, revoke), 04-break-glass (3 test: login, audit entry, UI indicator), 05-totp-recovery (4 test: recovery code üretimi, recovery sayfası, E2E key-loss uyarısı, audit entry), 06-websocket-realtime (2 test @slow: WS propagation, delete propagation), 07-report-generation (5 test: HTML response, içerik kontrolü, 50+ item limit, auth, UI), 08-mtls-client-cert (4 test: cert-olmadan 401, yanlış cert 401, admin UI, kullanıcı flag), 09-audit-log (5 test: create/read entry, UI sayfası, auth gereksinimi, pagination), 10-pipeline-editor (4 test: tab varlığı, ilişki API→graph, graph endpoint, global sayfa); `deploy/compose/docker-compose.e2e.yml` ephemeral stack (postgres+redis+minio+mailhog+api+web); `.github/workflows/e2e.yml` (main push + nightly 03:00 UTC + workflow_dispatch, playwright-report artifact); Makefile: e2e-up/e2e-down/e2e/e2e-ui hedefleri | ✅ Tamamlandı |
| **PR-PROD2: Test Coverage Uplift (Faz 11)** | `server/internal/httpapi/admin_k8s_test.go` (12 test: K8sClusterHandlers compile guard + auth-mode validation + name/URL validation + security invariants), `server/internal/httpapi/k8s_proxy_test.go` (9 test: K8sHandlers compile guard + resource types + RFC1123 namespace validation + proxy security invariants), `server/internal/httpapi/admin_report_test.go` (9 test: ReportHandlers compile guard + item count limit + formatTime/severityClass/metricPercent mirror tests + security invariants), `server/internal/clientcert/clientcert_test.go` (11 test: ExtractCertFromRequest 5 cases + error sentinels + distinctness + HeaderCertPEM constant + DefaultCertValidity); toplam 4 yeni test dosyası, 41 yeni test; `go test ./...` tüm 16 pakette geçiyor | ✅ Tamamlandı |
| **PR-SCAN: Secret Scanning / Leak Detection** | migration 00059 (secret_fingerprints: item_id+field_def_id+fingerprint BYTEA UNIQUE+scan_enabled, scan_detections: fingerprint_id+source_type+source_ref+detected_at+acknowledged_at, api_tokens scope extended to 'scan'); `httpapi/scan_handlers.go` ScanHandlers (UpsertFingerprint PUT /api/v1/items/{id}/scan — SHA-256 fingerprint from client; GetScanConfig GET; DeleteFingerprint DELETE; ScanContent POST /api/v1/security/scan — JWT or API token auth, batch fingerprint lookup, detection recording, audit security.leak_detected; ListDetections GET admin; AcknowledgeDetection POST admin); router wiring + main.go wiring; web: `api/api-tokens.ts` scope union extended to 'scan', `pages/admin/secret-scanning.tsx` admin page (detections list with ack button, scan tokens CRUD, endpoint URL copy, how-it-works guide); App.tsx route + app-shell.tsx ScanLine nav item; 6 test (compile guard + scanNullString + fingerprint hex validation × 5 + resolveScanActor no-bearer + invalid-JWT) | ✅ Tamamlandı |
| **PR-TF: Terraform Provider** | `terraform/` monorepo dizini — bağımsız Go modülü; `main.go` providerserver entrypoint; `internal/provider/provider.go` (ironstockProvider: url+api_token config, IronstockClient HTTP wrapper, folder/item/group resource + folder/item data source registry); `internal/resources/folder.go` (folderResource CRUD + folderDataSource, ironstockClient shared HTTP client with Bearer auth); `internal/resources/item.go` (itemResource CRUD: folder_id+name+item_type_id+description, itemDataSource); `internal/resources/group.go` (groupResource Create/Read/Delete); `examples/main.tf` (tam kullanım örneği: provider+folder hierarchy+group+item+data source+output); `docs/integrations/terraform.md` (kurulum, yapılandırma, 3 resource + 2 data source referansı, güvenlik notları) | ✅ Tamamlandı |
| **PR-BROWSER: Browser Extension** | `browser-extension/` dizini — Chrome Manifest V3; `manifest.json` (activeTab+storage+contextMenus permissions, service_worker background, content_scripts autofill.js); `src/lib/api-client.js` (chrome.storage.local token saklama, login+logout+search+getItem+listFolders, otomatik 401→refresh token yenileme); `src/background/service-worker.js` (context menu: "IronStock ile Doldur" + "IronStock'ta Ara: %s", runtime.onMessage dispatcher); `src/content/autofill.js` (findLoginForms: password+username input tespiti, fillCredentials: native setter+input/change event dispatch, overlay UI: dark theme credential picker, ESC kapatma); `src/popup/popup.html` + `popup.js` (login form: server URL+username+password+TOTP, vault view: debounced search, item click→autofill); `src/popup/options.html` (bağlantı durumu, oturum sonlandırma); `docs/integrations/browser-extension.md` (kurulum, kullanım, mimari, güvenlik, kısıtlamalar) | ✅ Tamamlandı |
| **PR-PROD7: Disaster Recovery + Backup (Faz 11)** | `scripts/backup.sh` (pg_dump + mc mirror + S3 upload, --db-only/--s3-upload flags, gzip sıkıştırma, manifest.json metadata, tar.gz arşivleme, retention temizliği); `scripts/restore.sh` (arşiv açma, S3'ten indirme, pg restore, MinIO mirror, kullanıcı onayı, doğrulama adımları); `deploy/k8s/cronjob-backup.yaml` (günlük 02:00 UTC CronJob, postgres:15-alpine, 10Gi PVC, 30 gün retention, backoffLimit:2); `docs/ops/backup.md` (yedekleme stratejisi, ortam değişkenleri, format, K8s CronJob, master key yönetimi, WAL archiving önerisi); `docs/ops/restore.md` (5 adımlı geri yükleme, 4 felaket senaryosu, doğrulama kontrol listesi, RTO<30dk RPO<24sa); `docs/ops/disaster-recovery.md` (quarterly restore drill runbook, tatbikat adımları+rapor şablonu, escrow prosedürü, iletişim planı, RTO/RPO tablosu) | ✅ Tamamlandı |
| **PR-PROD6: Dokümantasyon Tamamlama (Faz 11)** | 7 yeni döküman dosyası: `docs/ops/deployment.md` (tüm ortam değişkenleri referansı, K8s deployment YAML, PDB, NetworkPolicy, Docker Compose, migration, ilk kurulum adımları, sağlık kontrolleri); `docs/ops/monitoring.md` (Prometheus metrikleri tablosu, Grafana PromQL sorguları, alert'ler, pg_stat_statements, pprof kullanımı, log formatı+yönlendirme); `docs/security/threat-model.md` (şifreleme katmanları diyagramı, anahtar hiyerarşisi, zero-knowledge mimarisi, 8 tehdit vektörü+önlemler: DB sızıntısı/session hijack/brute-force/insider/supply chain/API/WS/mTLS, kriptografik algoritma tablosu, audit log alanları+action türleri, uyumluluk kontrol listesi); `docs/admin-guide/README.md` (kullanıcı yönetimi, roller, TOTP/WebAuthn/mTLS/IP/SSO yapılandırması, audit log, K8s cluster, export, bakım); `docs/api/README.md` (tüm 100+ endpoint tam referansı: auth, folder, item, vault, K8s proxy, group, tag, notification, pipeline, graph, SCIM, admin, WS akışı); `docs/integrations/ansible.md` (API token, dynamic inventory script, format); `docs/integrations/vault.md` (AppRole kurulumu, statik/dinamik secret, engine tablosu); `docs/integrations/scim.md` (Okta/Azure AD yapılandırması, attribute mapping) | ✅ Tamamlandı |
| **PR-PROD5: Performance + SLO (Faz 11)** | `docs/ops/slo.md` SLO hedef dökümanı (kullanılabilirlik ≥99.9%, REST p95<200ms, login p95<1s, WS <100ms, DB p95<50ms; hata bütçesi tablosu; Prometheus PromQL sorguları; 4 alert kuralı YAML); `e2e/load/scenarios.js` k6 yük testi (4 senaryo: login burst 50 VU, CRUD ramping 5→100 VU, search 30 VU, WebSocket 10→1000 VU; custom metrics: login/search/crud latency + error_rate; threshold'lar SLO ile eşleşir); `server/internal/config/config.go` PprofEnabled alanı (ENVANTER_PPROF_ENABLED, default false); `server/internal/httpapi/router.go` /debug/pprof/* route grubu (index/cmdline/profile/symbol/trace, PprofEnabled koşullu); `server/cmd/api/main.go` PprofEnabled wiring + warn log; `server/migrations/00060_pg_stat_statements.sql` (CREATE EXTENSION IF NOT EXISTS pg_stat_statements, N+1 sorgu tespiti için) | ✅ Tamamlandı |
| **PR-SIEM: Splunk HEC + Elastic ECS Forwarding** | migration 00058 (log_forwarding_configs CHECK constraint extended to include 'splunk'+'elastic'); `internal/logfwd/splunk.go` SplunkForwarder (HEC POST + exponential backoff retry, configID/Close/Send, ParseSplunkConfig); `internal/logfwd/elastic.go` ElasticForwarder (ECS field mapping: @timestamp+event.action+event.dataset+user.id+ironstock.*, Bulk API NDJSON, API key + Basic auth); `logfwd/forwarder.go` ParseConfig updated; `logfwd/manager.go` BuildForwarder updated; shared/types.ts: SplunkConfig+ElasticConfig interfaces, LogForwardingTargetType expanded; web `pages/admin/log-forwarding.tsx` Splunk/Elastic form fields + configToForm+formToConfig+configSummary handlers; 9 test (config parse + mock HTTP server send validation for both) | ✅ Tamamlandı |
