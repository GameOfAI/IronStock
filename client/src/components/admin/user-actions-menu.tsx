 ...
 * Ported from web/src/components/admin/user-actions-menu.tsx.
 */
  ...
  function handleTotpReset() {
    resetTotp.mutate(undefined, {
      onSuccess: () => {
        ...
        toast({
          title: 'TOTP sıfırlandı',
          description: `${user.username} bir sonraki girişte TOTP'yi yeniden kurmalı.`,
        });
      },
      onError: (err) => {
        toast({
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          <DropdownMenuCheckboxItem
            ...
            disabled={updateTotpReq.isPending || (isSelf && user.totp_required)}
            onSelect={(e) => {
              e.preventDefault();
              ...
              updateCertReq.mutate(
                { required: next },
                {
                  onSuccess: () =>
                    toast({
                      title: next
                        ? 'Sertifika zorunlu kılındı'
                        : 'Sertifika zorunluluğu kaldırıldı',
                      description: next
