/**
 * /admin/audit-log — Admin audit log görüntüleyici.
 * Ported from web/src/pages/admin/audit-log.tsx.
 */
...
export default function AdminAuditLogPage() {
  ...
  const { data, isLoading, isFetching, refetch, error } = useAuditLog(filters);

  useEffect(() => {
    if (error instanceof ApiError) {
      toast({ title: 'Audit log alınamadı', description: error.message, variant: 'destructive' });
    }
  }, [error, toast]);
    ...
    <div className="h-full overflow-auto p-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <AuditFilters
            value={filterValue}
            onChange={(next) => applyFilters(next === EMPTY_FILTERS ? EMPTY_FILTERS : next)}
